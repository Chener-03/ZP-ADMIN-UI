...
<div>
      ...
      <t-col :span="9">
        <t-form
          ...
          <t-form-item label="" name="username" style="min-width: 0;margin-right: 0">
            <t-input v-model="tableSearchFormData.username"
                     style="width: 150px"
                     clearable placeholder="用户名"></t-input>
          </t-form-item>
          <t-form-item label="" name="nameCn" style="min-width: 0;margin-right: 0">
            <t-input v-model="tableSearchFormData.nameCn"
                     style="width: 150px"
    ...
    <t-table row-key="id"
      ...
      <template #operation="{row}">
        <t-dropdown :options="dropDownList" @click="onDropListClick($event,row)">
          <t-space>
              <t-button theme="primary" size="small"  style="margin-right: 10px">
                管理
              </t-button>
          </t-space>
        </t-dropdown>
  ...
  <t-drawer v-model:visible="detailedInformationDrawer"
            header="详细信息" :placement="'right'"
            :destroy-on-close="true"
            :size="'50%'" :cancelBtn="null" @confirm="detailedInformationDrawer=false"
            :mode="'overlay'">
  ...
  <t-dialog
      ...
      <t-tab-panel :destroy-on-hide="false" :value="1" label="基础信息">
        ...
        >
          ...
          <t-form-item label="到期时间" name="expireTime">
            <t-date-picker
              @change="dc"
              v-model="dialogFormData.expireTime"
              placeholder="到期时间" />
          </t-form-item>
    ...
    <t-space size="small" style="float: right">
      ...
      <t-button theme="default" variant="base" type="reset" @click="resetDialogFormData"
                :disabled="!(dialogFormData.id===null)">重置</t-button>
    </t-space>
      ...
      //@ts-ignore
       request.post({
         ...
         if (r0 !== null && r0!== undefined) {
           ...
           }).catch(()=>{dialogFormSubmicStatus.value = false})
         }else{
...
const formRules = {
  ...
  email: [
    { email: true, message: '确定是邮箱?', type: 'warning' },
  ],
  phone:[
    { telnumber: true, message: '确定是手机号?', type: 'warning' },
  ]
}
