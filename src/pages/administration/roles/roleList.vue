<template>
  <t-button theme="primary" style="margin: 10px" @click="onAddClicked">新增</t-button>

  <t-button theme="primary" style="margin: 10px" @click="flushUiRoles" :loading="flushUiRoleBtnLoading"
    >重载界面权限列表</t-button
  >
  ...
  >
    ...
    <template #permissionEnNameList="{ row }">
      <t-select
        :value="transformPermissionList(row.permissionEnNameList)"
        :options="selectOp"
        :min-collapsed-num="3"
        @click="selectOpClick"
...
const dialogFormSubmit = () => {
  ...
  if (
    dialogForm.value.permissionEnNameList !== null &&
    dialogForm.value.permissionEnNameList !== undefined &&
    dialogForm.value.permissionEnNameList.length > 0
  ) {
