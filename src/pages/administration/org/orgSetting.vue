  ...
  <div>
    <t-space direction="vertical" style="width: 100%">
      ...
      <t-row :loading="dataLoading" :gutter="15">
        ...
        <t-col :span="4">
          <t-card
            ...
            :bordered="false"
            :hover-shadow="true"
            :subtitle="`${concurrentOrgExtInfo.subOrgCount}/${concurrentOrgInfo.subsidiaryNum}(个)`"
          ></t-card>
        </t-col>
        ...
        <t-col :span="4">
          <t-card
            ...
            :bordered="false"
            :hover-shadow="true"
            :subtitle="`${concurrentOrgExtInfo.activeUserCount}(人)`"
          ></t-card>
        </t-col>
      </t-row>
      ...
      <t-card :bordered="false" :hover-shadow="true" style="border-radius: 6px">
        ...
        <t-button style="margin-left: 10px" theme="primary" :loading="onAddUserBtnLoading" @click="onAddUserClick">{{
          addShow === 2 ? '新增' : '新增用户'
        }}</t-button>
        ...
        <t-button
          theme="primary"
          variant="outline"
          :loading="onFlushUserAuthBtnLoading"
          @click="flushUserAuth"
          style="float: right"
          >刷新用户权限</t-button
        >
    ...
    >
      ...
      <t-form :loading="submitBtnLoading" :data="orgInfoForm" ref="orgInfoFormRef" :rules="orgInfoFormRules">
        ...
        <t-form-item label="上级组织" name="parentId">
          <t-cascader
            v-model="orgInfoForm.parentId"
            :options="parentOrgTreeOption"
        ...
        <t-form-item label="角色" name="roleId">
          <t-select v-model="orgInfoForm.roleId" filterable placeholder="角色" clearable :options="remoteRoleOptions">
          </t-select>
        </t-form-item>
      </t-form>
    </t-drawer>
...
const onOrgEditClicked = () => {
  ...
  remoteUserOptions.value.push({
    // @ts-ignore
    label: `${concurrentOrgInfo.value.username}(${concurrentOrgInfo.value.userChName})`,
    userChName: concurrentOrgInfo.value.userChName,
    value: concurrentOrgInfo.value.mainUserId,
  });
};
