<template>
  <t-card :bordered="false">
    <t-form :data="searchForm">
      <t-row :gutter="10">
        <t-col :span="1.5">
          <t-select class="form-continer" clearable v-model="searchForm.type" :options="noticeTypeOption" placeholder="选择通知类型"></t-select>
        </t-col>
        <t-col :span="1.5">
    ...
    <t-table :data="tableData"
             ...
             :columns="columns">
      <template #op="{row}">
        <t-popconfirm  v-if="row.type==='3'"  theme="danger" content="确认将此通知置为过期?" @confirm="onRemoveNotifyClick(row)">
    ...
    <t-dialog
      ...
      <t-form ref="publishForm" :rules="publishFormRule"
              :data="publishFormData"
              :colon="true"
              :label-width="90"
              @submit="onPublish">
        <t-form-item label="通知类型" name="type">
          <t-select clearable v-model="publishFormData.type" :options="noticeTypeOption" placeholder="选择通知类型"></t-select>
        </t-form-item>
        ...
        <t-form-item label="渠道" name="ditch">
          <t-select clearable :disabled="publishFormData.type==='3'" v-model="publishFormData.ditch" multiple :options="noticeDitchOption" placeholder="选择通知渠道"></t-select>
        </t-form-item>
        <t-form-item label="结束时间" name="endTime"  >
          <t-date-picker :disabled="publishFormData.type!=='3'" enable-time-picker allow-input clearable v-model="publishFormData.endTime" />
        </t-form-item>
...
const publishFormRule = {
    ...
    { validator : (res)=>{
        return new Promise((resolve) => {
          if (publishFormData.value.type !== '3') {
            if (publishFormData.value.ditch === null || publishFormData.value.ditch === undefined
  ...
  containLink: [
    {required: true, message: '请选择是否包含链接', trigger: 'blur'}
  ],
  jmpLinkText: [
...
const onPublish = ()=>{
  publishForm.value.validate().then(success=>{
    if (success===true){
      if (publishFormData.value.users!==undefined
      ...
      if (publishFormData.value.ditch!==undefined
