    ...
    <t-card :bordered="false" hover-shadow>
      ...
      >
        ...
        <template #healthy="{ row }">
          <t-tag :theme="row.hosts.healthy === true ? 'success' : 'danger'">{{
            row.hosts.healthy === true ? '正常' : '异常'
          }}</t-tag>
        </template>
        ...
        <template #op="{ row }">
          <t-button variant="text" theme="primary" size="small" @click="jmpInfo(`${row.hosts.ip}:${row.hosts.port}`)"
            >详情</t-button
          >
        </template>
      </t-table>
